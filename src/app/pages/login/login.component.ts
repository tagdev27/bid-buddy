...
export class LoginComponent implements OnInit, OnDestroy {
    ...
    login() {
        ...
        if (checkBox) {
            firebase.auth().createUserWithEmailAndPassword(email, password).then(user => {
                //console.log(user)
                this.adminService.getUserData(email).then(async ud => {
                    //console.log(ud)
                    if (ud == null) {
                    ...
                    if(ud.role == 'Merchant'){
                        const m = await this.checkApprovalAccessForMerchant(ud.business_owner)
                        if(!m.approved){
                            this.displayMessage('Your account has not been approved yet. Please contact your Admin.', false)
                            return
                        }
                    }
    ...
    async checkApprovalAccessForMerchant(business_owner:string){
